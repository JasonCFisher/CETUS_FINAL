...
World* loadWorld(std::string location) {
	...
	//Declare Variables
	const int itemNum = 140;//set number of item files for array access - Will be 133 with full implementation
    ...
    std::string itemList[] = {"Lead Pipe.txt", "normBusinesses.txt", "normCar.txt", "altBusinesses.txt", "altCar.txt", "normBarBodies.txt", "normTables.txt",
		...
		"Ritual.txt", "Chips.txt", "Candybar.txt", "Specials.txt", "Phone.txt", "Block.txt"};
